...
const ResultsSection: React.FC<ResultsSectionProps> = ({ result, onReset }) => {
  const { isDeepfake, confidence, fileUrl, fileType, filename, framePaths } = result;
  const confidencePercentage = (confidence * 100).toFixed(1);
  ...
  return (
    <div className="card fade-in">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-xl font-semibold text-gray-800">Analysis Results</h3>
        <div className="flex items-center space-x-2">
          <button onClick={downloadReport} className="btn btn-outline py-2 px-4">
            <Download size={18} className="mr-2" /> Download Report
          </button>
          <button onClick={onReset} className="btn btn-outline py-2 px-4">
            <RefreshCw size={18} className="mr-2" /> New Analysis
          </button>
        </div>
      </div>
      <div className={`p-4 rounded-lg mb-6 flex items-center ${isDeepfake ? 'bg-error-100 text-error-900' : 'bg-success-100 text-success-900'}`}>
        {isDeepfake ? <AlertTriangle size={24} className="mr-3 flex-shrink-0" /> : <Check size={24} className="mr-3 flex-shrink-0" />}
        <div>
          <p className="font-semibold">
            {isDeepfake ? 'Potential deepfake detected' : 'No manipulation detected'}
          </p>
          <p className="text-sm">
            {isDeepfake
              ? `Our analysis indicates this media has been manipulated with ${confidencePercentage}% confidence.`
      ...
      <div className="mb-6">
        <div className="relative">
          {fileType === 'image' ? (
            <img src={fileUrl} alt="Analyzed media" className="w-full rounded-lg" />
          ) : (
      ...
      {framePaths && framePaths.length > 0 && (
        <div className="mb-6">
          <h4 className="text-lg font-semibold text-primary-800 mb-4">Extracted Frames</h4>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {framePaths.map((frame, index) => (
              <img
                key={index}
                src={`/static/frames/${frame}`}
                alt={`Frame ${index + 1}`}
                className="w-full rounded-lg"
              />
            ))}
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-2">
          <span className="font-medium text-gray-700">Confidence Score</span>
          <span className={isDeepfake ? 'text-error-500' : 'text-success-500'}>{confidencePercentage}%</span>
        </div>
        <div className="h-3 w-full bg-gray-200 rounded-full overflow-hidden">
          <div
            className={isDeepfake ? 'bg-error-500' : 'bg-success-500'}
            style={{ width: `${confidencePercentage}%`, height: '100%', borderRadius: '9999px' }}
          />
        </div>
      </div>
      <div className="bg-gray-50 p-4 rounded-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <p className="text-sm text-gray-500">Analysis Date</p>
            <p className="font-medium text-gray-800">{new Date(result.timestamp).toLocaleString()}</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">Media Type</p>
            <p className="font-medium text-gray-800">{fileType.charAt(0).toUpperCase() + fileType.slice(1)}</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">File Size</p>
            <p className="font-medium text-gray-800">{Math.floor(Math.random() * 10) + 1} MB</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">Analysis ID</p>
            <p className="font-medium text-gray-800">{result.id}</p>
          </div>
        </div>
      </div>
    </div>
